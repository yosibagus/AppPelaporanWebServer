...
$a = $tgl_awal;
$b = $tgl_akhir;
$q = mysqli_query($conn, "SELECT * FROM tb_laporan left join tb_kategori on tb_kategori.kd_kat = tb_laporan.kd_kat left join tb_user on tb_user.ktp_lap = tb_laporan.ktp_lap where tb_laporan.tgl_lap between '$a' and '$b'");
...
$pdf->ln(1);
$no=1;
$qu = mysqli_query($conn, "SELECT * FROM tb_laporan left join tb_kategori on tb_kategori.kd_kat = tb_laporan.kd_kat left join tb_user on tb_user.ktp_lap = tb_laporan.ktp_lap where tb_laporan.tgl_lap between '$a' and '$b'");
while($da=mysqli_fetch_array($qu)){
	...
	$pdf->MultiCell(3, -1, $da['ket_lap'], 1, 0, 'C');
